...
const ResumeView: React.FC = () => {
  return (
    <div id="resume-document" className="bg-white text-slate-900 p-8 md:p-12 shadow-none max-w-[900px] mx-auto font-serif leading-tight">
          ...
          {PROJECTS.map((proj, i) => (
            <div key={i}>
              <div className="flex justify-between text-[13px] font-bold">
                ...
                <span>{proj.date || "2024 - Present"}</span>
              </div>
              <ul className="list-disc ml-5 text-[12px] space-y-1 mt-1 text-slate-800">
                ...
                <li className="leading-tight font-bold">{proj.impact}</li>
              </ul>
            </div>
          ))}
        </div>
      ...
      {/* Achievements */}
      <section className="mb-6">
        <h2 className="text-lg font-bold uppercase border-b border-slate-900 mb-2">Achievements & Awards</h2>
        <ul className="list-none text-[12px] space-y-2 text-slate-800">
          {ACHIEVEMENTS.map((item, i) => {
             // Robust split for professional rendering
             const separatorIndex = item.indexOf(' - ');
             if (separatorIndex === -1) return <li key={i} className="leading-tight">• {item}</li>;
             
             const title = item.substring(0, separatorIndex);
             const description = item.substring(separatorIndex + 3);
